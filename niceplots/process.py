...
def parse_mapping(data, mapping, ctx, nbins):
    ...
    if mapping.strip() == "none":
        ...
        ms = {"bins": np.linspace(np.min(data) - 0.5, np.max(data) + 0.5, nbins + 1)}
    else:
...
def get_meta(var_idx, ctx, data, codebook):
    ...
    """
    # get question text
    meta = {"question": np.asarray(codebook[ctx["question_label"]], dtype=str)[var_idx]}

    # get missing code
    meta["missing_code"] = np.asarray(codebook[ctx["missing_label"]], dtype=int)[
        var_idx
    ]

    # get the code mapping (numerical answer to text conversion)
    variable_name = np.asarray(codebook[ctx["name_label"]], dtype=str)[var_idx]
    meta["mapping"], contains_no_answer = parse_mapping(
        data[variable_name],
        np.asarray(codebook[ctx["mapping_label"]], dtype=str)[var_idx],
        ctx,
        codebook["nbins - nice-plots"][var_idx],
    )

    # get additional plotting configurations (color scheme, etc.)
    for key in ctx["additional_codebook_entries"]:
        meta[key] = np.asarray(codebook[key + " - nice-plots"], dtype=str)[var_idx]
...
def process_data(data, codebook, ctx):
    ...
    category_labels, categories = parse_filter_functions(data, codebook, ctx)
    ...
    for block_id in np.unique(block_ids):
        ...
        for ii, cat in enumerate(categories):
            ...
            for var_idx in variable_indices:
                ...
                variable_name = np.asarray(codebook[ctx["name_label"]], dtype=str)[
                    var_idx
                ]
                ...
                meta, contains_no_answer = get_meta(var_idx, ctx, data, codebook)
                ...
                if contains_no_answer:
                    meta["no_answer"] = d[np.isclose(d, ctx["no_answer_code"])].size
                    d = d[np.logical_not(np.isclose(d, ctx["no_answer_code"]))]
        ...
        questions = [
            f"{plotting_data[category_labels[0]][xx]['meta']['question']}"
            for xx in range(len(variable_indices))
        ]
