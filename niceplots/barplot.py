...
def plot_nice_bar(ax, plotting_data, positions, ctx, height):
    ...
    for p_d in plotting_data:
        ...
        if "bins" in plotting_data[0]["meta"]["mapping"]:
            hist = np.histogram(d, bins=plotting_data[0]["meta"]["mapping"]["bins"])[0]
        else:
            hist = np.bincount(d)
            hist = hist[1:]
    ...
    all_category_colors = plt.get_cmap(plotting_data[0]["meta"]["color_scheme"])(
        np.linspace(0.15, 0.85, n_categories)
    )
    ...
    if hist.size != all_category_colors.shape[0]:
        raise Exception(
            "Number of colors is unequal to number of bins! Did you indicate the wrong 'no_answer' code."
        )
    ...
    for i in range(len(results)):
        ...
        bool_idx = np.append(widths, [0] * (all_category_colors.shape[0] - len(widths)))
        ...
        ax.barh(positions[i], widths, left=starts, height=height, color=category_colors)

        # add number indicating number of answers
        xcenters = starts + widths / 2.0
