...
def plot_timelines(xx, global_plotting_datas, ctx):
    ...
    n_questions = len(
        plotting_datas[first_key][list(plotting_datas[first_key].keys())[0]]
    )
    ...
    fig, ax = plt.subplots(
        ncols=1, nrows=n_questions, figsize=figsize, tight_layout=False
    )
    if n_questions == 1:
        ax = [ax]
    ...
    for ii in range(n_questions):
        ...
        for g, group in enumerate(filter_groups):
            means = [
                np.mean(plotting_datas[key][group][ii]["data"])
                for key in list(plotting_datas.keys())
            ]
            means = np.asarray(means)
            stds = [
                np.std(plotting_datas[key][group][ii]["data"])
                for key in list(plotting_datas.keys())
            ]
            ...
            # ax[ii].boxplot
            markers, caps, bars = ax[ii].errorbar(
                np.arange(len(means)),
                means - stds,
                yerr=stds,
                color=ctx["timeline_colors"][g],
                fmt="o",
                capsize=7,
                capthick=2,
                elinewidth=3,
        ...
        yticks = [
            plotting_datas[first_key][group][ii]["meta"]["mapping"][z]["code"]
        ...
        ax[ii].set_title(
            plotting_datas[first_key][group][ii]["meta"]["question"],
            fontsize=ctx["fontsize"],
        )

        if ii == 0:
            title_height = utils.get_render_size(
                plotting_datas[first_key][group][ii]["meta"]["question"],
                ctx,
                x_size=False,
            )
            if len(filter_groups) > 1:
                ax[ii].legend(
                    ncol=2,
                    bbox_to_anchor=(0, 1 + title_height + 0.2),
                    loc="lower left",
                    frameon=True,
